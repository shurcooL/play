// Play around with showing Go package source code
...
func run() error {
	...
	v, ok := os.LookupEnv("PLAY258_GOPROXY")
	if !ok {
		return fmt.Errorf("a Go module proxy must be provided via PLAY258_GOPROXY env var")
	}
...
func (h handler) ServeHTTP(w http.ResponseWriter, req *http.Request) error {
	...
	case req.URL.Path == "/-/style.css" ||
		req.URL.Path == "/-/tableofcontents.css" ||
		req.URL.Path == "/-/selectlistview.css":
		w.Header().Set("Content-Type", "text/css; charset=utf-8")
		return serveFile(w, req, filepath.Join("_data", req.URL.Path[len("/-/"):]))
	...
	case req.URL.Path == "/-/api/proxy" || strings.HasPrefix(req.URL.Path, "/-/api/proxy/"):
		...
		http.FileServer(http.Dir("/Users/dmitri/Downloads/proxywithstd")).ServeHTTP(w, req) // XXX.
		return nil
