...
func main() {
	...
	if *mode != "" {
		fd := annotate("/Users/Dmitri/Dropbox/Work/2013/GoLand/src/gist.github.com/7176504.git/main.go")
...
func annotate(name string) *bytes.Buffer {
	...
	_ = astutil.AddImport(file.fset, file.astFile, "gist.github.com/6418290.git")
	//file.fset = token.NewFileSet()
	ast.Walk(file, file.astFile)
	var fd = new(bytes.Buffer)
...
func new1Stmt(f *File, counter ast.Expr) ast.Stmt {
	s, err := gist5707298.ParseStmt("fmt.Println(gist6418290.GetParentFuncArgsAsString())")
	if err != nil {
		panic(err)
	}
	return s
