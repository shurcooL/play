...
func ExampleConstructQuery() {
...

	{
		type githubqlActor struct {
			Login     String
			AvatarURL URI // THINK: How to pass arguments?
			URL       URI
		}
	...
	// {repository(owner:"shurcooL-test"name:"test-repo"){issue(number:1){author{login,avatarUrl,url},publishedAt,lastEditedAt,editor{login,avatarUrl,url},body,reactionGroups{content,users{totalCount},viewerHasReacted},viewerCanUpdate,comments(first:1){nodes{databaseId,author{login,avatarUrl,url},publishedAt,lastEditedAt,editor{login,avatarUrl,url},body,reactionGroups{content,users{totalCount},viewerHasReacted},viewerCanUpdate},pageInfo{endCursor,hasNextPage}}}}}
